...
class CarRacing(gym.Env, EzPickle):
    ...
    def render(self, mode="human"):
        ...
        # provide an offset, so when the main surface is rotated, the drawn objects are not lost
        OFFSET = 300
        surf_x = WINDOW_W + 2 * OFFSET
        surf_y = WINDOW_H + 2 * OFFSET
        self.surf = pygame.Surface((surf_x, surf_y))
        ...
        trans_x = (scroll_x + PLAYFIELD) * zoom - self.surf.get_rect().centerx
        trans_y = (scroll_y + PLAYFIELD) * zoom - self.surf.get_rect().centery

        self.render_road(zoom, (-trans_x, -trans_y))
        self.car.draw(self.surf, zoom, (-trans_x, -trans_y), mode != "state_pixels")

        self.surf = pygame.transform.flip(self.surf, False, True)

        # rotating
        pos = (
            self.surf.get_width() // 2 - OFFSET,
            self.surf.get_height() // 2 - OFFSET + WINDOW_H / 4,
        )
        originPos = (self.surf.get_width() // 2, self.surf.get_height() // 2)

        surf_rect = self.surf.get_rect(
            topleft=(pos[0] - originPos[0], pos[1] - originPos[1])
        )
        offset_center_to_pivot = pygame.math.Vector2(pos) - surf_rect.center
        rotated_offset = offset_center_to_pivot.rotate(-angle)
        rotated_surf_center = (pos[0] - rotated_offset.x, pos[1] - rotated_offset.y)

        # get a rotated image
        rotated_surf = pygame.transform.rotate(self.surf, angle)
        rotated_surf_rect = rotated_surf.get_rect(center=rotated_surf_center)
        origin = rotated_surf_rect.topleft

        self.screen.fill(0)
        self.screen.blit(rotated_surf, origin)
    ...
    def render_road(self, zoom, translation):
        bounds = PLAYFIELD
        field = [
            (2 * bounds, 2 * bounds),
            (2 * bounds, 0),
        ...
        field = [
            (c[0] * zoom + translation[0], c[1] * zoom + translation[1]) for c in field
        ]
        gfxdraw.aapolygon(self.surf, field, (102, 204, 102))
        gfxdraw.filled_polygon(self.surf, field, (102, 204, 102))
